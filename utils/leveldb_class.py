...
class LevelDB:
        ...
        def getChainstateDB(self):
                return self.chainstatedb

        def getTxIndexDB(self):
                return self.txindexdb

        def __init__(self, nettype: str):
                print('nettype = %s' % nettype)
                bitcoin_path = '.bitcoin' if nettype == 'mainnet' else os.path.join('.bitcoin', nettype)
                bitcoin_fullpath = os.path.join(os.getenv('HOME'), bitcoin_path)
                print('bitcoin_fullpath = %s' % bitcoin_fullpath)
                self.blockdb = plyvel.DB(os.path.join(bitcoin_fullpath,"blocks/index"), compression=None)
                self.chainstatedb = plyvel.DB(os.path.join(bitcoin_fullpath,"chainstate"), compression=None)
                self.txindexdb = plyvel.DB(os.path.join(bitcoin_fullpath,"indexes/txindex"), compression=None)
        ...
        def applyObfuscationKey(self, data: bytes):
                obfuscation_key = self.getObfuscationKey()
                new_val = bytes(data[index] ^ obfuscation_key[index % len(obfuscation_key)] for index in range(len(data)))
                return new_val

        def getIteratorChainstateDB(self):
                it = self.chainstatedb.iterator(include_value=False)
                return it

        def getChainstateData(self, txn_hash_big_endian: bytes, out_index: int):
                jsonobj = {}
                key = b'C' + txn_hash_big_endian + b128_varint_encode(out_index)
                #print('key = %s' % key)
                value = self.chainstatedb.get(key)
        ...
        def getLastBlockFile(self):
                last_blockfile_number = int(binascii.hexlify(self.blockdb.get(b'l')[::-1]), 16)
                return last_blockfile_number

        def getBlockIndex(self, block_hash_bigendian: bytes):
                key = b'b' + block_hash_bigendian
        #        print(key)
                value = self.blockdb.get(key)
                jsonobj = {}
        ...
        def getTxnOffset(self, txn_hash_bigendian: bytes):
                key = b't' + txn_hash_bigendian
        #        print(key)
                value = self.txindexdb.get(key)
        ...
        def getRecentBlockHash(self):
                key = b'B'
        #        print(key)
                block_hash_b = self.chainstatedb.get(key)
